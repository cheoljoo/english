...
all:
	...
	if [ -n "$(FILE)" ]; then \
		   ...
		   if [ -f "$(FILE)" ]; then \
			...
			echo "Command 1: docker run -it --rm -v ./:/usr/src/app -e GEMINI_API_KEY=*** gemini-app gemini chat --model gemini-2.5-flash -y -p \"[FILE_CONTENT]\""; \
			if docker run -it --rm -v ./:/usr/src/app -e GEMINI_API_KEY="$(shell cat gemini.key)" gemini-app gemini --model gemini-2.5-flash -y -p "$$FILE_CONTENT"; then \
...
test-api:
	...
	@echo "Command: docker run --rm -v ./:/usr/src/app -e GEMINI_API_KEY=*** gemini-app gemini --help"
		@if docker run --rm -v ./:/usr/src/app -e GEMINI_API_KEY="$(shell cat gemini.key)" gemini-app gemini --help >/dev/null 2>&1; then \
		echo "✅ Gemini CLI is working"; \
		echo "Testing with a simple prompt..."; \
			   if docker run --rm -v ./:/usr/src/app -e GEMINI_API_KEY="$(shell cat gemini.key)" gemini-app gemini -y -p "Hello, just testing API connectivity. Please respond with 'OK'"; then \
